...
- **설명 줄바꿈**: `renderDescriptionInLines(text, terms, targetChars)` — 문장(`.!?` 뒤 공백)을 글자수 기준 그리디로 묶어 `<br />`. `max-w`(%)와 `targetChars`는 **한 쌍**(모바일 84%/25자, PC 90%/50자, 캡처 82%/35자)이며 텍스트 블록 래퍼에 `w-full`이 있어야 %가 카드 폭 기준으로 확정된다(없으면 래퍼가 가장 긴 문장 묶음 폭으로 줄어 항상 고아 줄 발생).
