...
- **라벨 폭**: 존별 기하 계산 — `right`는 `Math.max(80, VIEW_W - lx - 4)`, `left`는 `Math.max(80, lx - 4)`, `top`/`bottom`은 180(px, 160→180으로 상향 #4.24). 좌우 하한 80은 유지 — `R_OUTER` 확대로 `lx`가 커져도 하한을 올리면 라벨 박스가 카드 패딩을 넘어 짤리므로.
