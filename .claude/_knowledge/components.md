...
- **(#4.26) 캔버스 높이 `viewH`**: `VIEW_H`(664) 고정이 아니라 링 하단(`CY+R_OUTER`)과 라벨 추정 최하단(`estLabelHeight`: 한글 1em·그 외 0.62em로 줄 수 ≤3 추정) 중 큰 값 + 40px, 상한 `VIEW_H`. 6시 방향 라벨이 없으면 링 아래 빈 공간(≈100px) 회수. 라벨은 `labelMaxWOf(zone, lx)`를 렌더와 공유, 텍스트는 **종목 이름**(`seg.name`, 티커는 로고용)이며 `word-break:keep-all`. PC/모바일 폰트 분기는 `usePcPreview`.
