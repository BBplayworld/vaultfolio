...
## 자산 상세탭 공통 적용 판단 체크리스트 (강제 — 5탭 중 1개라도 수정 시)

> 상세탭(`stock-tab.tsx`/`crypto-tab.tsx`/`cash-tab.tsx`/`real-estate-tab.tsx`/`loan-tab.tsx`)은 하나만 고치고 끝내면 안 된다. 수정 범위를 아래 기준으로 먼저 분류하고, 해당하는 다른 탭도 같은 작업 단위에서 함께 고친다.

1. **하위 항목(증권사·거래소) 관련 수정인가?**
   → 주식(`SubStockCard`/`StockCard`의 `hasSubItems` 분기)과 암호화폐(`SubCryptoCard`/`CryptoCard`의 `hasSubItems` 분기)가 유일하게 하위 항목을 갖는 자산 쌍이다. 둘 중 하나를 고치면 **반드시 다른 하나도 동일 조건·동일 규칙으로 함께 수정**한다(예: 거래입력 노출 조건, 병합 대표 id 편집 disabled 조건, 나누기/그룹핑 로직).
2. **그 외(카드 헤더·거래입력 행·수정/삭제 액션·펼침 레이아웃 등) 공통 처리 수정인가?**
   → 5개 자산(현금·암호화폐·주식·부동산·대출) 전체에 동일하게 적용해야 하는지 판단한다. `ASSET_THEME` 토큰·카드 위계(`Card>CardHeader>CardContent`)·액션 버튼 배치 등은 5탭 통일이 원칙([qa-full-test-plan.md](./qa-full-test-plan.md) F-ASSET 참조)이므로, 한 탭에서만 스타일/동작을 바꾸면 나머지 탭도 함께 갱신한다. 자산 고유 특성(주식의 스크린샷 모드, 대출의 완납 리스트 등)은 예외로 남긴다.
3. 위 두 축 중 하나라도 걸리면 **해당 탭들을 같은 커밋/작업 단위에서 함께 수정**하고, 필요 시 `qa-full-test-plan.md` F-* 항목도 같이 갱신한다.

---

## 주식 계산·집계는 항상 종목(티커) 기준 종합이 기본 (강제)

> 배경: S-4.31 연말 절세 시뮬레이션이 `assetData.stocks`를 증권사별 개별 행 그대로 순회해, 동일 종목을 여러 증권사에 나눠 보유하면 종목마다 별개 세액을 계산하는 오류가 있었다. 실제 세법(양도소득세 등)은 증권사가 아니라 **종목 단위**로 손익을 합산해 판정한다.

- 주식 관련 신규 기능(집계·계산·리포트 등)을 구현할 때, **명시적으로 "증권사별로 보여줘/계산해줘"라는 요청이 없는 한 항상 티커 기준으로 종합한 값을 사용**한다. 증권사별로 쪼개진 개별 `Stock` 로우를 그대로 순회해 종목별 결과를 내는 구현은 기본값으로 금지.
- 재사용: `asset-detail-tabs.tsx`의 `groupStocksByTicker`(티커 기준 그룹핑) + `mergeStockGroup`(그룹 합산 대표 `Stock` 생성 — 수량 합산, 평단·매입환율은 수량 가중평균)이 상세 > 주식 탭 "전체" 카테고리 뷰가 쓰는 표준 패턴이다. **컴포넌트 파일에서 호출하는 코드는 이 두 함수를 그대로 재사용**한다.
- `src/lib/`(컴포넌트에 의존하면 안 되는 계층)에서 동일 종합이 필요하면 `asset-detail-tabs.tsx`를 import하지 말고 — 이 파일은 `src/app/(main)/_components/` 아래 컴포넌트 파일이라 `lib`가 역참조하면 계층 위반이다 — `tax-utils.ts`의 `groupForeignStocksByTicker`처럼 **동일 로직(티커 그룹핑 + 수량 가중평균 병합)을 그 파일 안에 동형으로 둔다**. 새로 이런 함수를 또 만들게 되면 셋 이상 반복되는 시점에 공용 위치(예: `src/lib/asset/stock-grouping.ts`)로 승격을 검토한다.
- 판단 기준: 화면에 "종목 카드가 몇 개 보이는가"가 아니라 "세액·손익·통계 계산의 단위가 무엇인가"다. 리스트 UI는 증권사별 하위 카드(`SubStockCard`)를 유지해도 되지만, 그 위의 종합 계산(세금·총손익 등)은 항상 티커 단위여야 한다.

---

## 공지·팁 콘텐츠 작성 3원칙 (강제 — `notice.tsx`·`feature-tip-box.tsx` 등 사용자 대상 안내 문구 전부)

> 배경: S-4.32 홈 "기능 활용 팁" 박스 도입 시 정립. 기존 공지사항(`notice.tsx`)에도 동일하게 소급 적용되는 규칙이다.

1. **기능 위치·이동 경로 명시**: "어디서 확인하는지"를 문구에 텍스트로 짚고(예: "세금 관리에서", "상세 탭의 주식에서"), 클릭 시 실제로 그 위치로 이동해야 한다. 이동이 불가능한 안내(단순 팝업)는 피하고, 가능하면 항상 클릭 → 해당 기능 직행으로 연결한다.
2. **필요시 SVG(아이콘)로 인지성 극대화**: 텍스트만 나열하지 않고 `lucide-react` 아이콘으로 한눈에 성격(TIP/신규/세금 등)을 구분한다.
3. **핵심 위주의 간결함**: 스크롤 없이 한 번에 훑히는 분량을 넘기지 않는다. 한 문장 요약 + 필요한 최소 부연 정도로 제한하고, 조건·예외를 나열하는 긴 설명은 상세 페이지로 미룬다.
