...
- ⚙ 기능(feature) 카탈로그는 `APP_FEATURES`(14개, #4.24에 `share-card-portfolio` 추가. 백업·피드백·설정·기기 동기화는 카탈로그 제외 대상)에 신규 공용 기능 추가 시 등록 필수. **`isNew`는 이번 릴리스 신규만** — 지난 릴리스 항목의 `isNew`는 해제해야 다음 신규가 `pickRecommendedFeature`의 `.find(isNew)`에 걸린다(#4.24에 `tax-simulator` isNew 해제·`stocks-xray`/`share-card-portfolio` isNew 설정). 방문 기록은 `navigation-context.tsx`의 `navigate()` 단일 지점 `recordVisit(viewToKey(v))` + `share-card`(action형)는 `top-bar.tsx`의 `trigger-open-share-card` 리스너가 별도 기록.
- 👤 (신규 공지 미열람 상태) 홈 진입 → **공지 카드가 백업·세금보다 먼저** 노출·클릭 시 인증카드 포트폴리오 다이얼로그 오픈 / 공지 열람 후 → 백업 30일 이상 미실행 시 백업 카드·클릭 시 파일 다운로드 / 세금 신고 대상 자산 보유 시 세금 카드·클릭 시 `#tax` / 자산 30일 이상 미갱신 시 최신화 카드·클릭 시 입력 시트 / 위 모두 없으면 "종목 유형 분석(X-Ray)" 또는 "포트폴리오 인증카드" NEW 팁
