...
- 👤 **모바일 셸**(#4.24): `DialogContent`가 모바일에서 **상하좌우 12px 인셋**(`left-3 right-3` + `top/bottom-[max(0.75rem,env(safe-area-inset-*))]`, `translate-x/y-0`로 중앙 배치 해제, `rounded-2xl border shadow-lg`) — 가장자리로 공용 `DialogOverlay`(`bg-black/70 backdrop-blur-sm`)가 살짝 비침. `DialogDescription` 숨김(`hidden sm:block`), 헤더 `px-3 pt-3 pb-2`, X 버튼 `top-3`(다이얼로그가 노치 아래로 인셋돼 safe-area 패딩 불필요), 프리뷰 컨테이너 `px-0`. 데스크톱(`sm:`)은 중앙 배치 복귀(760px, 94dvh, `p-4`, `sm:top/left-[50%] sm:translate-x/y-[-50%] sm:bottom/right-auto`). 셸 클래스는 캡처 대상 아님(R32 무관)
...
- 👤 상단: **닉네임 미노출**(2026-08-08 제거) → `StockSummaryHeader`(주식 탭과 동일 컴포넌트, `DetailSummaryHeader`의 `screenshotMode` 분기) — 배경 박스 없이 아래 비중 바·리스트와 좌우 여백 동일(`px-2`=8px, 카드 전체 좌우 오프셋 20px), "총 주식 평가금액" 히어로 숫자는 `ASSET_THEME_SHOT.summaryValue`(`text-xl`=20px, 상세 탭 모바일과 동일 — 2026-09 정렬) + 평가손익(수익금 `text-base`·수익률 `text-sm`). 원/달러 셀렉터·오늘 등락 칩은 미노출. **금액 전부(헤더·종목 리스트·"그 외 N종목") 전체 금액**(`840,180,000원` 형태, 상세 탭과 동일, `share-card.tsx`의 `mask`→`formatCurrency`) — 축약(`8.4억원`) 미사용
- 👤 비중 바 — **상위 7종목(SHOT_MAX, 2026-08) 색 구간 + 회색 `그 외`(#9ca3af) 구간** + 바로 아래 **범례**(색점 + 종목명 + 비중%, `그 외 N종목` 포함). 범례는 `ASSET_THEME_SHOT.legendGrid`(`grid-cols-2`)·`legendText`(`text-sm`) 사용, `sm:` 금지(R32) — 프리뷰·캡처 두 인스턴스 모두 `screenshotMode` 경로라 동일. 저장 PNG 결정성은 화면 밖 680 고정 캡처 인스턴스가 담당. 비중 바·범례·리스트를 감싸는 래퍼는 **배경색 없음**(카드 전체 배경과 통일, 패딩 `p-3.5`만 간격 계산용으로 유지)
...
- ⚙ **텍스트·아이콘 크기 — 프리뷰/캡처 분리**(2026-09) — 프리뷰(`responsive`)는 `ASSET_THEME_SHOT`(상세>주식 탭 모바일값: 14/9/10/20/16/14px, `icon` `size-6`). **캡처(저장 PNG, `shotBig`=`!responsive`)는 `ASSET_THEME_SHOT_BIG`** = 위 폰트·아이콘을 `SHOT_BIG_SCALE`배 한 결과를 하드코딩(현재 1.42 → 20/13/14/28/23/20px, `icon` `size-[34px]`). `SHOT_BIG_SCALE`(theme.ts)은 문서용 배율 상수 — **Tailwind JIT가 `text-[Npx]`를 소스에서 스캔하므로 CSS 변수/런타임 계산 불가**, 값 바꾸면 BIG 값을 base×SCALE 반올림으로 재계산해 교체(theme.ts 주석 base 표). `shotBig`은 `screenshotMode`와 별개 플래그(재결속 시 프리뷰 펼침 부활). 전부 `sm:` 없는 고정값(R32). 예외: `cardHeader`/`cardTriggerButton`/`legendGrid` 간격. 하드코딩 본문은 `bodyText` 공용 키.
- ⚙ **포트폴리오 도넛 라벨 — 프리뷰/캡처 분리** — 라벨 `<span>`은 인라인 `fontSize: rLabelFont`. 프리뷰(`responsive`): 링 전체 `transform: scale(<1)` 축소 → `rLabelFont = Math.max(15, 12/scale)`(모바일 실효 ~12px, "분야 구성" 14px과 비슷). 캡처(저장 PNG, `!responsive`): `rLabelFont = Math.round(12 * SHOT_BIG_SCALE)`(theme.ts import; 1.42→17). **종목명 짤림 방지는 프리뷰·캡처 공통**: `line-clamp-3`, `labelMaxW` 하한 좌우 `max(64, …)`(9/3시 가용폭 초과 시 링 좌표계 이탈→화면 밖 짤림이라 64)·top/bottom `208`, 프리뷰 `scale`이 좌우 `PREVIEW_SIDE_INSET`(8px)씩 여백, `rGap` 프리뷰 `round(rLabelFont*4.5)`·캡처 `round(rLabelFont*5)`. `VIEW_W`/`VIEW_H`/`CX`/`CY`/`R_*` 불변 → 아트보드·`pixelRatio` 유지(R32). `PortfolioSectorBar`는 `big`(=`shotBig`)로 캡처 시 `text-sm`→`text-[20px]`·막대/색점 `2.5`→`14px`.
