...
- 👤 헤더~범례~리스트~푸터 간격 통일: 헤더~범례·범례~리스트·리스트~푸터 실제 노출 간격 **36px**로 동일(숨은 패딩까지 계산해 마진값 역산 — 2026-09 28→36px 상향, 래퍼 `py-[22px]` + 리스트 `mt-9`, 상세>주식 탭과 동일 간격), 카드 최상단↔헤더 값·푸터↔카드 최하단 간격도 대칭(`pt-2`/`pb-2`). 금액 표시 스위치 끄면 금액류만 `••••` 마스킹(비중%·수익률%는 항상 노출), 푸터 좌측에 "시크릿에셋"(`text-xs text-foreground`) + 도메인(`secretasset.xyz`) — **날짜 없음**(#4.24 후속 제거)
