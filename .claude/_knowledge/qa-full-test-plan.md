...
- 👤 헤더~범례~리스트~푸터 간격 통일: 헤더~범례·범례~리스트 실제 노출 간격 28px로 동일(숨은 패딩까지 계산해 마진값 역산), 카드 최상단↔헤더 값·푸터↔카드 최하단 간격도 대칭(`pt-2`/`pb-2`). 금액 표시 스위치 끄면 금액류만 `••••` 마스킹(비중%·수익률%는 항상 노출), 푸터 좌측에 "시크릿에셋"(`text-xs text-foreground`) + 도메인(`secretasset.xyz`) — **날짜 없음**(#4.24 후속 제거)
- ⚙ 데이터는 `useFilteredStockData("all")` 단일 출처 — 주식 탭과 캐시 키 공유(중복 fetch 없음). 종목 파생값은 `computeStockMetrics`
- ⚙ **종목 행 텍스트 크기** — `ASSET_THEME_SHOT`의 `cardInfoName`/`cardAmountMain`(14px)·`iconInitial`(9px)·`badge`(10px)는 상세>주식 탭 **모바일**과 동일(프리뷰가 상세 탭과 같게 보이도록, 2026-09). 히어로 `summaryValue`(24px)·`profitAmount`(18px)·`profitRate`(16px)만 공유 이미지 강조로 큼. 전부 `sm:` 없는 고정값(R32).
