...
- 👤 비중 바 — **상위 7종목(SHOT_MAX, 2026-08) 색 구간 + 회색 `그 외`(#9ca3af) 구간** + 바로 아래 **범례**(색점 + 종목명 + 비중%, `그 외 N종목` 포함). 캡처 폭 고정이라 범례는 `ASSET_THEME_SHOT.legendGrid`(`grid-cols-2`)·`legendText`(`text-sm`) 사용, `sm:` 금지(R32). 비중 바·범례·리스트를 감싸는 래퍼는 **배경색 없음**(카드 전체 배경과 통일, 패딩 `p-3.5`만 간격 계산용으로 유지)
