...
- 👤 헤더~범례~리스트~푸터 간격 통일: 헤더~범례·범례~리스트·리스트~푸터 실제 노출 간격 **40px**로 동일(숨은 패딩까지 계산해 마진값 역산 — 2026-09 28→40px 상향, 래퍼 `py-[26px]` + 리스트 `mt-10`, 상세>주식 탭과 동일 간격), 카드 최상단↔헤더 값·푸터↔카드 최하단 간격도 대칭(`pt-2`/`pb-2`). 금액 표시 스위치 끄면 금액류만 `••••` 마스킹(비중%·수익률%는 항상 노출), 푸터 좌측에 "시크릿에셋"(`text-xs text-foreground`) + 도메인(`secretasset.xyz`) — **날짜 없음**(#4.24 후속 제거)
- ⚙ 데이터는 `useFilteredStockData("all")` 단일 출처 — 주식 탭과 캐시 키 공유(중복 fetch 없음). 종목 파생값은 `computeStockMetrics`
- ⚙ **텍스트·아이콘 크기 — 프리뷰/캡처 분리**(2026-09) — 프리뷰(`responsive`)는 `ASSET_THEME_SHOT`. 상세>주식 탭 모바일보다 **의도적으로 ~2px 작다**(2026-09 축소 — 프리뷰에서 주식 현황이 포트폴리오 도넛 라벨(실효 ~12px)보다 커 보여 낮춤): 본문 `text-xs`(12), `summaryValue` `text-[17px]`, `profitAmount` `text-sm`(14), `iconInitial` `text-[8px]`, `badge` `text-[9px]`, `icon` `size-5`(20), footer `text-[10px]`/`text-xs`. **캡처(저장 PNG, `shotBig`=`!responsive`)는 `ASSET_THEME_SHOT_BIG`** = 캡처 기준 base(14/9/10/20/16/14·icon 24)를 `SHOT_BIG_SCALE`배 한 결과를 하드코딩(현재 1.46 → 20/13/15/29/23/20px, `icon` `size-[35px]`). 프리뷰(SHOT)와 base 분리 — 캡처 크기 불변. `SHOT_BIG_SCALE`(theme.ts)은 문서용 배율 상수 — **Tailwind JIT가 `text-[Npx]`를 소스에서 스캔하므로 CSS 변수/런타임 계산 불가**, 값 바꾸면 BIG 값을 base×SCALE 반올림으로 재계산해 교체(theme.ts 주석 base 표). `shotBig`은 `screenshotMode`와 별개 플래그(재결속 시 프리뷰 펼침 부활). 전부 `sm:` 없는 고정값(R32). 예외: `cardHeader`/`cardTriggerButton`/`legendGrid` 간격. 하드코딩 본문은 `bodyText` 공용 키.
- ⚙ **포트폴리오 도넛 라벨 — 프리뷰/캡처 분리** — 라벨 `<span>`은 인라인 `fontSize: rLabelFont`. 프리뷰(`responsive`): 링 전체 `transform: scale(<1)` 축소 → `rLabelFont = Math.max(15, 12/scale)`(모바일 실효 ~12px, `PortfolioSectorBar` 프리뷰 `text-xs`(12px)와 동일). 캡처(저장 PNG, `!responsive`): `rLabelFont = Math.round(12 * SHOT_BIG_SCALE)`(theme.ts import; 1.46→18). **종목명 짤림 방지는 프리뷰·캡처 공통**: `line-clamp-3`, `labelMaxW` 하한 좌우 `max(64, …)`(9/3시 가용폭 초과 시 링 좌표계 이탈→화면 밖 짤림이라 64)·top/bottom `208`, 프리뷰 `scale`이 좌우 `PREVIEW_SIDE_INSET`(8px)씩 여백, `rGap` 프리뷰 `round(rLabelFont*4.5)`·캡처 `round(rLabelFont*5)`. `VIEW_W`/`VIEW_H`/`CX`/`CY`/`R_*` 불변 → 아트보드·`pixelRatio` 유지(R32). `PortfolioSectorBar` 텍스트는 프리뷰 `text-xs`(2026-09 `text-sm`→`text-xs`) / 캡처(`big`=`shotBig`) `text-[20px]`·막대·색점 `size-[15px]`.
