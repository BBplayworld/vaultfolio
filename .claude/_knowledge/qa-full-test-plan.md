...
- 👤 **모바일 확대**(#4.24): `DialogContent`가 모바일 `w-screen × h-[100dvh]`(테두리·라운드 제거) 전체화면, `DialogDescription` 숨김(`hidden sm:block`), 헤더·제어바 패딩 축소(`px-3 py-2`), 프리뷰 컨테이너 좌우 여백 0(`px-0`). 데스크톱(`sm:`)은 현행 유지(760px, 94dvh, `p-4`). 헤더 상단·X 버튼은 `pt-[max(0.875rem,env(safe-area-inset-top))]`/`[&_[data-slot=dialog-close]]:top-[max(0.875rem,env(safe-area-inset-top))]`로 노치 여백 확보(데스크톱 `top-4`). 이 셸 클래스들은 캡처 대상이 아니라 다이얼로그 셸이라 `sm:`·`env()` 허용(R32 무관)
