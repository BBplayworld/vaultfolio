...
- ⚙ **텍스트·아이콘 크기 — 프리뷰/캡처 분리**(2026-09) — 프리뷰(`responsive`)는 `ASSET_THEME_SHOT`. 상세>주식 탭 모바일보다 **의도적으로 ~2px 작다**(2026-09 축소 — 프리뷰에서 주식 현황이 포트폴리오 도넛 라벨(실효 ~12px)보다 커 보여 낮춤): 본문 `text-xs`(12), `summaryValue` `text-[17px]`, `profitAmount` `text-sm`(14), `iconInitial` `text-[8px]`, `badge` `text-[9px]`, `icon` `size-5`(20). **캡처(저장 PNG, `shotBig`=`!responsive`)는 `ASSET_THEME_SHOT_BIG`** = 캡처 기준 base(14/9/10/20/16/14·icon 24)를 `SHOT_BIG_SCALE`배 한 결과를 하드코딩(현재 1.46 → 20/13/15/29/23/20px, `icon` `size-[35px]`). 프리뷰(SHOT)와 base 분리 — 캡처 크기 불변. `SHOT_BIG_SCALE`(theme.ts)은 문서용 배율 상수 — **Tailwind JIT가 `text-[Npx]`를 소스에서 스캔하므로 CSS 변수/런타임 계산 불가**, 값 바꾸면 BIG 값을 base×SCALE 반올림으로 재계산해 교체(theme.ts 주석 base 표). `shotBig`은 `screenshotMode`와 별개 플래그(재결속 시 프리뷰 펼침 부활). 전부 `sm:` 없는 고정값(R32). 예외: `cardHeader`/`cardTriggerButton`/`legendGrid` 간격. 하드코딩 본문은 `bodyText` 공용 키.
- ⚙ **포트폴리오 도넛 라벨 — 프리뷰/캡처 분리** — 라벨 `<span>`은 인라인 `fontSize: rLabelFont`. 프리뷰(`responsive`): 링 전체 `transform: scale(<1)` 축소 → `rLabelFont = Math.max(12, 12/scale)`(항상 실효 12px, `PortfolioSectorBar` 프리뷰 `text-xs`(12px)와 동일 — 하한이 과거 15였을 때는 PC처럼 `scale≈1`인 경우 15px로 고정돼 나머지 본문(12px)보다 커 보이는 회귀가 있었다, 2026-09 수정). 같은 스케일 컨테이너 안의 `BrandMark`(조각 안 ETF 브랜드 배지) 폰트는 **동일 보정을 적용하면 안 된다** — 칩(`size`)이 scale과 무관한 고정 px 좁은 컨테이너라 `/scale`로 폰트만 키우면 넘친다(2026-09 시도 후 회귀, 롤백). 캡처(저장 PNG, `!responsive`): `rLabelFont = Math.round(12 * SHOT_BIG_SCALE)`(theme.ts import; 1.46→18). **종목명 짤림 방지는 프리뷰·캡처 공통**: `line-clamp-3`, `labelMaxW` 하한 좌우 `max(64, …)`(9/3시 가용폭 초과 시 링 좌표계 이탈→화면 밖 짤림이라 64)·top/bottom `208`, 프리뷰 `scale`이 좌우 `PREVIEW_SIDE_INSET`(8px)씩 여백, `rGap` 프리뷰 `round(rLabelFont*4.5)`·캡처 `round(rLabelFont*5)`. `VIEW_W`/`VIEW_H`/`CX`/`CY`/`R_*` 불변 → 아트보드·`pixelRatio` 유지(R32). `PortfolioSectorBar` 텍스트는 프리뷰 `text-xs`(2026-09 `text-sm`→`text-xs`) / 캡처(`big`=`shotBig`) `text-[20px]`·막대·색점 `size-[15px]`.
