...
interface Props { rowClassName: string }

export function CloudSyncMenuEntry({ rowClassName }: Props) {
  ...
  return (
    <>
      <button type="button" className={rowClassName} onClick={() => setOpen(true)}>
        <Cloud className="size-5 text-primary shrink-0" />
        <span className="flex-1 font-medium">클라우드 동기화</span>
        {cs.status === "armed" && <RefreshCw className={`size-4 text-primary ${cs.syncing ? "animate-spin" : ""}`} />}
        <span className="rounded-md bg-primary/15 px-2 py-0.5 text-xs font-semibold text-primary">Plus</span>
      </button>

      <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) resetLocal(); }}>
        <DialogContent className="sm:max-w-md max-h-[85vh] overflow-y-auto touch-pan-y">
          <DialogHeader>
            ...
            <DialogDescription>
              자산을 암호화해 백업하고 다른 기기와 맞춥니다. 서버는 암호문만 보관하며 금고 암호 없이는 열 수 없습니다.
            </DialogDescription>
          </DialogHeader>
